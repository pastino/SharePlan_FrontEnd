...
const Fiveth = ({ navigation }) => {
  const editDivision = navigation.getParam("editDivision");
  const fivethStartDay = navigation.getParam("fivethStartDay");
  const division = navigation.getParam("division");

  const originDDay = navigation.getParam("originDDay");
  const [selectedDay, setSelectedDay] = useState();

  const [fivethSelectedLastDay, setFivethSelectedLastDay] = useState();

  const selectedDayHandle = (day) => {
    setFivethSelectedLastDay(day.dateString);
    ...
    let startDate = moment(fivethStartDay).format("YYYY-MM-DD");
    ...
    <View>
      ...
      >
        ...
        <TouchableOpacity
          onPress={() =>
            division === "create"
              ? navigation.navigate("GoalStackFour", { fivethSelectedLastDay })
              : division === "edit"
              ? navigation.navigate("GoalEdit", { fivethSelectedLastDay })
              : editDivision === "edit"
      ...
      <CalendarList
        current={fivethStartDay}
        minDate={fivethStartDay}
